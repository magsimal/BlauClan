  ...
  function mount() {
    ...
    const app = createApp({
      ...
      template: `
        ...
        <li v-for="s in visiblePlaceSuggestions" :key="s.geonameId" class="list-group-item list-group-item-action" @pointerdown.stop.prevent="applyPlace(s)">{{ s.name }}<span v-if="s.postalCode"> ({{ s.postalCode }})</span><span v-if="s.adminName1">, {{ s.adminName1 }}</span> {{ s.countryCode }}</li>
                        <li class="list-group-item list-group-item-action" @pointerdown.stop.prevent="useTypedPlace" data-i18n="useExactly">Use Exactly</li>
                      </ul>
                    </div>
                  </div>
                  <div class="form-row">
